...
function Emergency({ user }) {
  ...
  if (loading) return <div className="rounded-xl bg-white p-6 shadow-sm">Carico scheda emergenza...</div>;
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl bg-medink p-6 text-white shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-teal-100">Accesso medico NFC/QR</p>
            <h1 className="mt-1 text-3xl font-bold">Scheda emergenza paziente</h1>
            <p className="mt-2 text-slate-200">Questa e la schermata che medico o ospedale devono leggere velocemente.</p>
          </div>
          <button onClick={handlePrint} className="rounded-lg bg-white px-5 py-3 text-sm font-bold text-medink hover:bg-teal-50">
            Stampa / PDF
          </button>
        </div>
      </section>
